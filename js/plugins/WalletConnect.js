    ...
    async function connectAGW() {
        ...
        try {
            ...
            if (typeof window.Privy === 'undefined') {
                throw new Error('Privy SDK not loaded. Please refresh the page and try again.');
            }
            ...
            if (!privyClient) {
                try {
                    ...
                    throw new Error('Failed to initialize Privy client: ' + privyError.message);
                }
            }

            // Try to connect using Privy SDK
            try {
                await privyClient.connect();
                
                // Get the connected account
                const user = privyClient.getUser();
                if (!user || !user.wallet) {
                    throw new Error('No wallet found after connection');
                }
            ...
            } catch (privyConnectError) {
                console.error('Privy connection error:', privyConnectError);
                
                // Fallback to popup method if SDK fails
                console.log('Falling back to popup connection method...');
                await connectAGWPopup();
            }

        } catch (error) {
            console.error('AGW connection error:', error);
            showNotification('Failed to connect Abstract Global Wallet: ' + error.message, 'error');
            setButtonState(button, false);
        }
    }

    // Fallback popup connection method
    async function connectAGWPopup() {
        const button = document.getElementById('wallet-connect-btn');
        ...
        try {
            ...
            console.log('Opening Privy connection popup:', privyUrl);
            ...
            console.error('AGW popup connection error:', error);
            throw error;
        }
    }
